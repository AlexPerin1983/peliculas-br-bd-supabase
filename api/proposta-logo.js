...
export default async function handler(request, response) {
    ...
    try {
        ...
        if (dataImage) {
            response.setHeader('Content-Type', dataImage.contentType);
            response.setHeader('Content-Length', dataImage.image.byteLength);
            response.setHeader('Cache-Control', 'public, max-age=3600, s-maxage=86400');
            return response.status(200).end(dataImage.image);
        }
