...
interface PixStatusRequest {
    sessionId: string;
}

interface AbacateTransparentStatusResponse {
    data?: {
        id: string;
        status: string;
        paidAmount?: number;
...
serve(async (req) => {
    ...
    try {
        ...
        const body = (await req.json()) as PixStatusRequest;
        if (!body.sessionId) {
            throw new Error('sessionId obrigatorio');
        }

        const response = await abacateRequest<AbacateTransparentStatusResponse>(
            'v2',
            `/transparents/check?id=${encodeURIComponent(body.sessionId)}`
        );
        ...
        return new Response(
            JSON.stringify({
                ...
                paidAmount: response.data?.paidAmount || null,
                amount: response.data?.amount || null,
                expiresAt: response.data?.expiresAt || null,
                devMode: isProductionAbacateKey()
                    ? false
                    : Boolean(response.data?.devMode)
            }),
            {
                headers: corsHeaders,
                status: 200
