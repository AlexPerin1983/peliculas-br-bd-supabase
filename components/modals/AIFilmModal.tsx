...
const AIFilmModal: React.FC<AIFilmModalProps> = ({ isOpen, onClose, onProcess, isProcessing, provider }) => {
    ...
    const processingMessages: Record<string, string> = {
        ...
        extracting: 'Extraindo especificacoes...',
    ...
    const TabButton: React.FC<{ tab: 'text' | 'image' | 'audio'; icon: string; children: React.ReactNode }> = ({ tab, icon, children }) => (
        <button type="button" onClick={() => handleTabChange(tab)} disabled={isProcessing} className={`flex-1 px-3 py-2 text-sm font-semibold rounded-md transition-colors duration-200 flex items-center justify-center gap-2 ${activeTab === tab ? 'bg-slate-800 dark:bg-slate-700 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
            <i className={icon}></i>
            {children}
        </button>
    );
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Preencher Pelicula com IA" footer={footer}>
            <form id="aiFilmForm" onSubmit={handleSubmit} className="space-y-4" aria-busy={isProcessing}>
                <div className="flex space-x-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                    ...
                    {provider === 'gemini' && <TabButton tab="audio" icon="fas fa-microphone">Audio</TabButton>}
                </div>
                ...
                <div className="relative min-h-[250px] flex flex-col justify-center items-center">
                    {isProcessing && (
                        <div className="absolute inset-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center z-10 rounded-lg" aria-live="polite">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Isso pode levar alguns segundos</p>
                        </div>
                    )}
                    ...
                    {processingStage === 'error' && !isProcessing && (
                        <div className="absolute inset-0 bg-white/95 dark:bg-slate-900/95 flex flex-col items-center justify-center z-10 rounded-lg p-4">
                            ...
                            <p className="text-red-700 dark:text-red-300 font-medium text-center mb-4">{errorMessage || 'Erro ao processar. Tente novamente.'}</p>
                            <button type="button" onClick={handleRetry} className="px-4 py-2 bg-slate-800 dark:bg-slate-700 text-white rounded-md hover:bg-slate-700 dark:hover:bg-slate-600 flex items-center gap-2">
                    ...
                    {activeTab === 'text' && (
                        <>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 self-start">Ex: <span className="italic">Película G5, preço 100 reais o metro, bloqueia 99% UV e 50% calor.</span></p>
                            <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Descreva ou cole as informações da película aqui..." className="w-full h-48 p-2 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-slate-500 focus:border-slate-500 sm:text-sm" disabled={isProcessing} />
                        </>
                    )}
                    ...
                    {activeTab === 'image' && (
                        <div className="w-full text-center flex flex-col items-center justify-center">
                                    ...
                                    <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                                        {imagePreviews.map((previewUrl, index) => (
                                            <div key={index} className="relative group aspect-video">
                                                ...
                                                <button type="button" onClick={() => removeImage(index)} disabled={isProcessing} className="absolute top-1 right-1 h-6 w-6 bg-black/60 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Remover imagem ${index + 1}`}>
                            ...
                            {imageFiles.length < MAX_IMAGES && (
                                <>
                                    <div onDragEnter={(e) => handleDragEvent(e, true)} onDragLeave={(e) => handleDragEvent(e, false)} onDragOver={(e) => handleDragEvent(e, true)} onDrop={handleDrop} className={`w-full p-6 border-2 border-dashed rounded-lg transition-colors ${isDragging ? 'border-slate-500 bg-slate-100 dark:bg-slate-800' : 'border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800'} ${isProcessing ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                                        <label htmlFor="image-upload" className={isProcessing ? 'cursor-not-allowed' : 'cursor-pointer'}>
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Pode ser um print de catálogo ou foto da embalagem.</p>
                                        </label>
                                    </div>
                                    <div className="mt-3 flex gap-2 justify-center">
                                        <label htmlFor="camera-capture" className={`px-4 py-2 bg-slate-700 dark:bg-slate-600 text-white text-sm font-semibold rounded-md hover:bg-slate-600 dark:hover:bg-slate-500 transition-colors flex items-center gap-2 ${isProcessing ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
                                            <i className="fas fa-camera"></i>
                                            Tirar Foto
                                        </label>
                                    </div>
                                    <input type="file" accept="image/*" capture="environment" onChange={(e) => handleImageFiles(e.target.files)} className="hidden" id="camera-capture" disabled={isProcessing || imageFiles.length >= MAX_IMAGES} />
                                </>
                            )}
                        ...
                        <div className="w-full flex flex-col items-center justify-center text-center">
                            {audioUrl ? (
                                ...
                                <>
                                    ...
                                    <p className="text-slate-600 mb-4">Gravando... fale os dados da película.</p>
                                    <button type="button" onClick={stopRecording} className="px-6 py-2 bg-red-600 text-white rounded-full font-semibold shadow-md">Parar gravação</button>
                                </>
                            ) : (
                                <>
                                    <i className="fas fa-microphone text-3xl text-slate-400 mb-3"></i>
                                    <p className="text-slate-600 mb-4">Clique no botão para começar a gravar o áudio com os dados da película.</p>
                                    <button type="button" onClick={startRecording} disabled={isProcessing} className="px-6 py-2 bg-slate-800 dark:bg-slate-700 text-white rounded-full font-semibold shadow-md hover:bg-slate-700 dark:hover:bg-slate-600">Iniciar gravação</button>
                                </>
                            )}
                        </div>
                    )}
