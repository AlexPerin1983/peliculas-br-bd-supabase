...
const AIFilmModal: React.FC<AIFilmModalProps> = ({ isOpen, onClose, onProcess, isProcessing, provider }) => {
    ...
    const processingMessages: Record<string, string> = {
        ...
        extracting: 'Extraindo especificações...',
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Preencher Película com IA" footer={footer}>
                    ...
                    {activeTab === 'text' && (
                        <>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 self-start">
                                Ex: <span className="italic">Película G5, preço 100 reais o metro, bloqueia 99% UV e 50% calor.</span>
                            </p>
                            <textarea
                                value={text}
                                onChange={(e) => setText(e.target.value)}
                                placeholder="Descreva ou cole as informações da película aqui..."
                                    ...
                                    >
                                        <label htmlFor="image-upload" className={isProcessing ? 'cursor-not-allowed' : 'cursor-pointer'}>
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                                Pode ser um print de catálogo ou foto da embalagem.
                                            </p>
                                        </label>
                                    </div>
