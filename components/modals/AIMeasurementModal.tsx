        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Preenchimento Automatico com IA" footer={footer} fullScreenOnMobile>
            <form id="aiForm" onSubmit={handleSubmit} className="space-y-4" aria-busy={isProcessing}>
                ...
                <div className="relative min-h-[250px] flex flex-col justify-center items-center">
                    ...
                    {activeTab === 'text' && (
                        <>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 self-start">Ex: <span className="italic">5 janelas de 1.20 por 2.10 na sala, 2 vidros fixos 0.80x1.50 no escritorio.</span></p>
                            <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Descreva ou cole as medidas aqui..." className="w-full h-48 p-2 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-slate-500 focus:border-slate-500 sm:text-sm" disabled={isProcessing} />
                        </>
                    )}
                        ...
                        <div className="w-full text-center flex flex-col items-center justify-center">
                            {imagePreviews.length > 0 && (
                                <div className="w-full mb-2">
                                    ...
                                    {imageFiles.length < MAX_IMAGES && <label htmlFor="image-upload" className="mt-2 text-sm text-slate-700 hover:text-slate-900 font-semibold cursor-pointer inline-block">Adicionar mais imagens... ({imageFiles.length}/{MAX_IMAGES})</label>}
                                </div>
                            )}
                            ...
                            {imageFiles.length < MAX_IMAGES && imagePreviews.length === 0 && (
                                <div onDragEnter={(e) => handleDragEvent(e, true)} onDragLeave={(e) => handleDragEvent(e, false)} onDragOver={(e) => handleDragEvent(e, true)} onDrop={handleDrop} className={`w-full p-6 border-2 border-dashed rounded-lg transition-colors ${isDragging ? 'border-slate-500 bg-slate-100 dark:bg-slate-800' : 'border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800'} ${isProcessing ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                                    <label htmlFor="image-upload" className={isProcessing ? 'cursor-not-allowed' : 'cursor-pointer'}>
                                        ...
                                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Ate {MAX_IMAGES} imagens. Pode ser uma foto, um print ou um rascunho.</p>
                                    </label>
                                </div>
                            )}
