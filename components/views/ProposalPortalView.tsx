...
const buildLocalDemoPortal = (): PublicProposalPortal => {
    ...
    return {
        ...
        proposals: [
            { id: 101, proposalOptionName: 'Opção Premium', nomeArquivo: 'proposta-premium.pdf', totalPreco: 4850, totalM2: 36.8, date: new Date().toISOString(), status: 'pending' },
    ...
    return (
        <div className="min-h-screen bg-[#f4f7fb] pb-28 text-slate-900 sm:pb-10" style={{ '--portal-brand': brand } as React.CSSProperties}>
                ...
                <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(300px,.75fr)]">
                            ...
                            {data.proposals.map((proposal, index) => {
                                ...
                                return <article key={proposal.id} className={`rounded-2xl border bg-white p-4 shadow-sm transition sm:p-5 ${active ? 'border-blue-500 ring-4 ring-blue-500/10' : 'border-slate-200'}`}>
                                    <button type="button" onClick={() => setSelectedId(proposal.id!)} className="w-full text-left">
                                        <div className="flex items-start gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-sm font-black ${active ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{index + 1}</span><div className="min-w-0 flex-1"><h3 className="truncate text-base font-extrabold text-slate-950">{proposal.proposalOptionName || proposal.nomeArquivo || `Proposta ${index + 1}`}</h3><p className="mt-0.5 text-xs text-slate-500">{proposal.totalM2 ? `${proposal.totalM2.toFixed(2).replace('.', ',')} m²` : 'Detalhes completos no PDF'}</p></div><p className="shrink-0 text-lg font-black text-slate-950">{currency.format(proposal.totalPreco || 0)}</p></div>
                                    </button>
                    ...
                    <aside className="hidden lg:block">
                        <div className="sticky top-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-400">Sua decisão</p><h2 className="mt-1 text-xl font-bold text-slate-950">{selected?.proposalOptionName || 'Proposta selecionada'}</h2><p className="mt-2 text-2xl font-black text-slate-950">{currency.format(selected?.totalPreco || 0)}</p>
                            <div className="mt-5 grid gap-2"><button disabled={isClosed} onClick={() => setModal('approved')} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-emerald-600 text-sm font-extrabold text-white disabled:opacity-50"><ThumbsUp className="h-4 w-4" /> Aprovar</button><button disabled={isClosed} onClick={() => setModal('negotiation')} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-blue-600 text-sm font-extrabold text-white disabled:opacity-50"><HandCoins className="h-4 w-4" /> Negociar</button><button disabled={isClosed} onClick={() => setModal('rejected')} className="flex h-12 items-center justify-center gap-2 rounded-xl border border-red-200 text-sm font-extrabold text-red-600 disabled:opacity-50"><ThumbsDown className="h-4 w-4" /> Recusar</button></div>
                        </div>
                    </aside>
                </div>
                ...
                <section id="conversa" className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div className="max-h-[420px] space-y-3 overflow-y-auto bg-slate-50/70 p-4 sm:p-5">
                        {data.messages.length === 0 ? <div className="py-8 text-center text-sm text-slate-400"><MessageCircle className="mx-auto mb-2 h-7 w-7" /> Ainda não há mensagens. Se precisar, fale com a empresa por aqui.</div> : data.messages.map(item => <div key={item.id} className={`flex ${item.sender_type === 'client' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[86%] rounded-2xl px-4 py-3 text-sm leading-6 ${item.sender_type === 'client' ? 'rounded-br-md bg-blue-600 text-white' : 'rounded-bl-md bg-white text-slate-700 shadow-sm'}`}>
                            {item.kind !== 'message' ? <p className="mb-1 text-[10px] font-black uppercase tracking-[0.12em] opacity-75">{item.kind === 'approved' ? 'Proposta aprovada' : item.kind === 'rejected' ? 'Proposta recusada' : 'Contraproposta enviada'}</p> : null}
                    ...
                    {!data.portal.expired ? <div className="flex gap-2 border-t border-slate-100 p-3 sm:p-4"><textarea value={message} onChange={event => setMessage(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void sendMessage(); } }} rows={1} placeholder="Digite sua mensagem…" className="min-h-11 flex-1 resize-none rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-blue-500" /><button type="button" disabled={busy || !message.trim()} onClick={() => void sendMessage()} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-white disabled:opacity-50"><Send className="h-4 w-4" /></button></div> : null}
                </section>
            </main>
            ...
            <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 p-3 pb-[calc(env(safe-area-inset-bottom,0px)+.75rem)] backdrop-blur-xl lg:hidden">
                {isClosed ? <div className="flex h-12 items-center justify-center gap-2 rounded-xl bg-slate-100 text-sm font-bold text-slate-600"><CheckCircle2 className="h-5 w-5" /> {statusLabel[data.portal.status]}</div> : <div className="mx-auto grid max-w-xl grid-cols-3 gap-2"><button onClick={() => setModal('approved')} className="flex h-12 flex-col items-center justify-center rounded-xl bg-emerald-600 text-[11px] font-extrabold text-white"><ThumbsUp className="h-4 w-4" /> Aprovar</button><button onClick={() => setModal('negotiation')} className="flex h-12 flex-col items-center justify-center rounded-xl bg-blue-600 text-[11px] font-extrabold text-white"><HandCoins className="h-4 w-4" /> Negociar</button><button onClick={() => setModal('rejected')} className="flex h-12 flex-col items-center justify-center rounded-xl bg-red-50 text-[11px] font-extrabold text-red-600"><ThumbsDown className="h-4 w-4" /> Recusar</button></div>}
            </div>

            {modal && selected ? <ResponseModal kind={modal} proposalName={selected.proposalOptionName || selected.nomeArquivo || 'Proposta'} proposalValue={selected.totalPreco || 0} busy={busy} onClose={() => setModal(null)} onSubmit={payload => void submitResponse(payload)} /> : null}
        </div>
    );
};
