...
import { summarizeProposalExpenses } from './proposalExpenses';
import { calculatePricingAreaM2 } from './pricingArea';
import { calculateProposalAdjustmentAmounts } from './proposalAdjustments';

type DiscountType = ProposalDiscount;

interface CalculateProposalTotalsParams {
...
/**
 * Cálculo puro dos totais de uma proposta a partir de medidas + películas + ajustes.
 *
 * Extraído de `useProposalTotals` para poder ser usado fora de um componente React
 * (ex.: gerar o orçamento em segundo plano para o fluxo "segue orçamento" do
 * WhatsApp, sem depender da proposta aberta na tela).
 */
export function calculateProposalTotals({
    measurements,
    films,
    generalDiscount
}: CalculateProposalTotalsParams): Totals {
